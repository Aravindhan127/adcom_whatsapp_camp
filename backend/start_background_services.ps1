...
# 1. Start Redis Server (DISABLED: Using Remote Redis)
...
Write-Host "[2/3] Starting Celery Worker (adcom_worker)..." -ForegroundColor Yellow
...
# 3. Start Celery Beat (Required for Scheduling)
Write-Host "[3/3] Starting Celery Beat (Scheduler)..." -ForegroundColor Yellow
Start-Process -FilePath "python.exe" -ArgumentList "-m celery -A app.core.celery_app beat --loglevel=info" -WindowStyle Normal

Write-Host "Success: All services triggered in the background." -ForegroundColor Green
Write-Host "You can verify the worker and scheduler in your browser via the Campaign Manager UI." -ForegroundColor Cyan
